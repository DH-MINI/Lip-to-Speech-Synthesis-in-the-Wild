...
class MultiDataset(Dataset):
    def __init__(self, data, mode, max_v_timesteps=155, min_window_size=30, max_window_size=50, max_text_length=150, augmentations=False, num_mel_bins=80, fast_validate=False):
        ...
        self.file_paths, self.file_names = self.build_file_list(data, mode)
        self.f_min = 55.
        self.f_max = 7500.
    ...
    def build_file_list(self, lrs3, mode):
        ...
        ## LRS3 file lists##
        with open(f"./data/grid/grid_{mode}.txt", "r") as file:
            content = file.read()
        for file in content.splitlines():
            file_list.append(file)
            paths.append(f"{lrs3}/{file}")
    ...
    def build_tensor(self, frames):
        ...
        for i, frame in enumerate(frames):
            transform = transforms.Compose([
                ...
                # Crop(crops[i]),
                # transforms.RandomCrop(88,88) if self.augmentations else transforms.CenterCrop(88,88),
                transforms.RandomCrop(88) if self.augmentations else transforms.CenterCrop(88),
    ...
    #             break
    #     cap.release()
    #     audio, _ = librosa.load(file_path.replace('video', 'audio') + '.wav', sr=16000)
